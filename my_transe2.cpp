...
int combination_restriction = 500;
...
//	entityTotal = 20867;
	...
	//add
    FILE *fin_w;
//	int weight_num = relationTotal;  // cannot do that
//	int weight_num = 1563;
    fin_w = fopen((inPath + "weight_alpha.txt").c_str(), "r");

    tmp = fscanf(fin_w, "%d", &weight_num); // right
    auto_weight = (float *)calloc(weight_num, sizeof(float));
//    fin_w = fopen((inPath + "weight_alpha.txt").c_str(), "r");  // postion wrong
//    tmp = fscanf(fin_w, "%d", &weight_num);  //do that, but postion error
    for(int i = 0;i<weight_num;i++){
        int weightId;
        float weight;
        tmp = fscanf(fin_w, "%d", &weightId);
        tmp = fscanf(fin_w, "%f", &weight);
        auto_weight[weightId] = weight;
//        printf("%d\n", weight_num);
//        printf("%f\n", weightId);
//        printf("%f\n", weight);
//        printf("%f\n", auto_weight[weightId]);
...
float calc_sum(int e1, int e2, int rel) {
	...
	int lastr = rel * dimension;
        	for (int ii=0; ii < dimension; ii++) {
//            		sum += fabs(auto_weight[rel] * (relationVec[lastr + ii] + entityVec[last2 + ii] - entityVec[last1 + ii]));
...
//            		sum += fabs(auto_weight[rel] * (entityVec[last1 + ii] + relationVec[lastr + ii] - entityVec[last2 + ii]));
...
//    if (auto_weight[rel_a]<0.001 && auto_weight[rel_b]<0.001){
//        return;
//    }
//    float temp = auto_weight[rel_a];
//    float temp2 = temp;
	for (int ii=0; ii  < dimension; ii++) {
		float x;
//		x = auto_weight[rel_a] * (relationVec[lastar + ii] + entityVec[lasta2 + ii] - entityVec[lasta1 + ii]);
		x = (relationVec[lastar + ii] + entityVec[lasta2 + ii] - entityVec[lasta1 + ii]);
//		x = (entityVec[lasta1 + ii] + relationVec[lastar + ii] - entityVec[lasta2 + ii]);
//		x = auto_weight[rel_a] * (entityVec[lasta1 + ii] + relationVec[lastar + ii] - entityVec[lasta2 + ii]);
		if (x > 0)
			x = transeAlpha;
		else
			x = -transeAlpha;
...
//		entityVec[lasta2 + ii] += x;
        ...
        relationVec[lastar + ii] -= auto_weight[rel_a] * x;
        entityVec[lasta1 + ii] += auto_weight[rel_a] * x;
        entityVec[lasta2 + ii] -= auto_weight[rel_a] * x;

//        x = auto_weight[rel_a] * (entityVec[lastb2 + ii] + relationVec[lastbr + ii] - entityVec[lastb1 + ii]);
        x = (entityVec[lastb2 + ii] + relationVec[lastbr + ii] - entityVec[lastb1 + ii]);
//        x = auto_weight[rel_b] * (entityVec[lasta1 + ii] + relationVec[lastar + ii] - entityVec[lasta2 + ii]);
		if (x > 0)
			x = -transeAlpha;
		else
			x = transeAlpha;
...
//		entityVec[lastb2 + ii] += x;
        ...
        relationVec[lastbr + ii] -=  auto_weight[rel_b] * x;
        entityVec[lastb1 + ii] += auto_weight[rel_b] * x;
        entityVec[lastb2 + ii] -= auto_weight[rel_b] * x;
	}
}
...
//		entityVec[lasta2 + ii] += x;
        ...
        x = (entityVec[lastb1 + ii] + relationVec[lastbr + ii] - entityVec[lastb2 + ii]);
//        x = auto_weight[rel_b] * (entityVec[lasta1 + ii] + relationVec[lastar + ii] - entityVec[lasta2 + ii]);
        if (x > 0)
            x = -transeAlpha;
        else
...
//        if (number < 0.1 * transeTrainTimes){
//            printf("%d \n", number);
//            gradient(e1_a, e2_a, rel_a, e1_b, e2_b, rel_b);
//            min_res = res;
//        }else{
...
void out_transe(string iter = "") {
		FILE* f2 = fopen((outPath + "relation2vec" + iter + ".bern_me1").c_str(), "w");
		FILE* f3 = fopen((outPath + "entity2vec" + iter + ".bern_me1").c_str(), "w");
		for (int i=0; i < relationTotal; i++) {
